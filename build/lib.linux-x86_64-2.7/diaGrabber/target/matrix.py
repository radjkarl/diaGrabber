...
class _matrix(object):
	...
	def __init__(self, basis_dim, merge_dim, derivate_dim, sourceClass):
	...
	def _checkMinMax(self,basis_dim, nDim):
		...
		if len(dims) > 0:
			...
			for i in range(nDim):
				if basis_dim[i]._take_all_values:
					basis_dim[i].include_from_to = min_max[j]
					j+=1
				print "set range for %s to %s" %(basis_dim[i].name,
					basis_dim[i].include_from_to)
	...
	def eval2dGaussianDistribution(self, dEB_types=[], which_merge_dim = 0):
		...
		self.plot_overlay[0].append(tuple(peak_coord))
		self.plot_overlay[6].append("Mittelpunkt")#x-,y-list

		self.plot_overlay[5].append("deviation= %s" %relDevianceTh2Real)
		self.plot_overlay[5].append("maxRad= %s" %mean_max_radian)
		self.plot_overlay[5].append("max/min-d-factor= %s" %mean_maxMin_dEB)
		#add rings
		for d in range(1,len(dEB_limit)):
			self.plot_overlay[1].append(zip(*border_coords[d]))#x-,y-list
			self.plot_overlay[6].append("d_EB= %s" %(dEB_types[d]) )#x-,y-list
	...
	def transformDim(self, rebuild_matrix = True):
		...
		#for i in range(self.nDim):
			...
			#transfrom range in includeFromTo
				for j in range(2):
					self.basis_dim[i].include_from_to[j] = \
					self.basis_dim[i].transform.get(self.basis_dim[i].include_from_to[j])
		...
		for m in range(self.nMerge):
			...
			for i in range(len(points)):
				...
				for position, intensity in positionsIntensities:
					...
					(replace_value, do_replace)= self.merge_dim[m].merge._get(merge[m][i],
...
class coarseMatrix(_matrix):
	...
	def __init__(self, basis_dim, merge_dim, derivate_dim, sourceClass):
		super(coarseMatrix, self).__init__(basis_dim, merge_dim, derivate_dim, sourceClass)
		#self.basis_dim = basis_dim
...
+---------+---------+-----------+
	'''
	def __init__(self, basis_dim, merge_dim, derivate_dim, sourceClass):
		super(fineMatrix, self).__init__(basis_dim, merge_dim, derivate_dim, sourceClass)
		#self.basis_dim = basis_dim
