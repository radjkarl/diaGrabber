...
class matPlotLib(_plot):
	...
	def heatMap(self, title = ""):
		...
		for  m in range(len(self.merge_index)):
			...
			self.ax[-1].set_xlabel(self.basis_dim[0].name)
			self.ax[-1].set_ylabel(self.basis_dim[1].name)
	...
	def _getAxisRange(self):
		axis_range = []
		for i in range(self.nDim):
			axis_range.append(self.basis_dim[i]._include_from_to[0])#min
			axis_range.append(self.basis_dim[i]._include_from_to[1])#max
